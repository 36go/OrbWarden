...
<template>
  <div
    ...
    class="orbs-nitro-status inline-flex min-h-[4.5rem] w-fit max-w-full items-center gap-3 rounded-full border border-blue-500/20 bg-blue-500/10 px-4 py-2.5 text-blue-700 transition-colors hover:bg-blue-500/[0.13] dark:text-blue-300"
  >
    ...
    <div class="min-w-0">
      <div v-if="showOrbs" class="flex items-center gap-1.5">
        ...
        <span class="whitespace-nowrap text-xl font-semibold leading-none tabular-nums" aria-live="polite">
          {{ questsStore.orbsBalance == null ? 'â€”' : questsStore.orbsBalance.toLocaleString() }}
        </span>
        <Button
          variant="ghost"
          size="icon"
